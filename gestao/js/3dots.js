...
$(document).ready(function () {
    ...
    $('.btn_submit_form').click(function () {
        $(this).closest('#form_admin').submit();
...
function validaFormulario(form) {
    ...
    $(form).find("input , select, textarea").each(function () {
        if (typeof $(this).attr('required') != "undefined") {
            ...
            // campos vázios
            if (!$(this).hasClass("opcional") && $(this).val() == "" && $(this).attr("type") != "hidden" && $(this).attr("id") != "conteudo" && $(this).attr("id") != "descricao_home" && $(this).attr("id") != "descricao_sobre") {
                var erros = {
                    'campo': $(this).attr("id"),
            ...
            if ($(this).attr("id") == "conteudo") {
                var valueConteudo = $(this).val();
                var trimValueConteudo = $.trim($(valueConteudo).text());

                if (trimValueConteudo == "") {
                    var erros = {
                        'campo': $(this).attr("id"),
                        'msg': "Este campo está vázio."
                    };
            ...
            // Textarea Descriçao Home
            if ($(this).attr("id") == "descricao_home") {
                var valueDescricao = $(this).val();
                var trimValueDescricao = $.trim($(valueDescricao).text());

                if (trimValueDescricao == "") {
                    var erros = {
                        'campo': $(this).attr("id"),
                        'msg': "Este campo está vázio."
                    };
            ...
            // Textarea Descriçao Sobre
            if ($(this).attr("id") == "descricao_sobre") {
                var valueDescricaoHome = $(this).val();
                var trimValueDescricaoHome = $.trim($(valueDescricaoHome).text());

                if (trimValueDescricaoHome == "") {
                    var erros = {
                        'campo': $(this).attr("id"),
                        'msg': "Este campo está vázio."
                    };
            ...
            // Input Data com campo não obrigatório
            if ($(this).attr("id") == "datahora_fim") {
                var dateTime = luxon.DateTime;
                var dataAtual = dateTime.now().toString().substring(0,16);

                if ($(this).val() != "" && $(this).val() < dataAtual) {
                    var erros = {
                        'campo': $(this).attr("id"),
                        'msg': "Data/hora não pode ser menor que data/hora atual."
                    };
                    camposVazios.push(erros);
                }
            } else {
    ...
    if (camposVazios.length > 0) {
        ...
        for (var j = 0; j < camposVazios.length; j++) {
            $(form + " select" + "#" + camposVazios[j].campo).parent().append("<span style='color: #ed1c24; font-size: 12px; position: absolute; left: 0; bottom: -20px; display: block; line-height: 20px;' class='erro-form'>" + camposVazios[j].msg + "</span>");
        }
        for (var j = 0; j < camposVazios.length; j++) {
