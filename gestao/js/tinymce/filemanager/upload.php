...
if (!empty($_FILES) || isset($_POST['url'])) {
    if (isset($_POST['url'])) {
        ...
        fclose($handle);
        $_FILES['file'] = array('name' => basename($_POST['url']),
            'tmp_name' => $temp,
            'size' => filesize($temp),
    ...
    if (in_array(fix_strtolower($extension), $ext)) {
        ...
        if (in_array(fix_strtolower($extension), $ext_img)) $is_img = TRUE; else $is_img = FALSE;
...
if (isset($_POST['submit'])) {
    $query = http_build_query(array('type' => $_POST['type'],
        'lang' => $_POST['lang'],
        'popup' => $_POST['popup'],
