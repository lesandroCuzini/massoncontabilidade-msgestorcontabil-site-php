$(document).ready(function () {
    $(document).on("click", "#imagens-upload div.container-img > span.apagar", function () {
        if (confirm("Deseja realmente apagar essa imagem?")) {
            var urlAjax = gestao_url + "/quem-somos-form.html?id=" + $("#id_registro").val();
            $.ajax({
                method: "POST",
                url: urlAjax,
                data: {
        ...
        success: function (resposta) {
            ...
            for (var i = 0; i < resposta.length; i++) {
                $("#imagens-upload").append(
                    ...
                    "<div class='img'>" +
                    "<img src='" + site_uploads + "/quem-somos/galeria/small/" + resposta[i].url_imagem + "'></div>" +
                    "<span class='apagar' data_id_objeto_foto='" + resposta[i].id_quem_somos_fotos + "'>Apagar</span>" +
                    "</div>");
            }
            $(".loading").hide();
            $(".loader").hide();
