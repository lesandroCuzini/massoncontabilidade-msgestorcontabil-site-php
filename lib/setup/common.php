...
function paginacao($total_reg, $pag_atual, $rpp, $url)
{
    ...
    if ($total_pag > 1) {
        if ($pag_atual < 5) {
            ...
            if ($total_pag > 5)
                $str_paginacao .= "<span>...</span>\n<a href=\"$url" . $total_pag . "\">" . $total_pag . "</a>\n<a href=\"$url" . $frente . "\" class='frente icon'></a>";

        } elseif ($pag_atual > $total_pag - 4) {
            $i = 1;
            $str_paginacao .= "<a href=\"$url" . $atras . "\" class='voltar icon'></a>\n<a href=\"$url" . $i . "\">" . $i . "</a>\n<span>...</span>\n";
...
/**
 * Corta a imagem , modificado por 3dots Galvani
 *
 * @param $imgSrc
 ...
 * @return null
 */
function CroppedThumbnail($imgSrc, $caminho_thumb, $thumbnail_width, $thumbnail_height)
{
    ...
    if ($mimeType == "image/png" || $mimeType == "image/x-png" || $mimeType == "image/gif") {
        uploadImagemBgTransparente($imgSrc, $caminho_thumb, $thumbnail_width, $thumbnail_height);
    } else {
 ...
 */
function uploadImagemBgTransparente($src_origem, $src_destino, $width, $height) {
...
function uploadArquivo($file_name,$file_tmp_name, $dir_file, $prefixo_nome, $thumb = false, $dir_thumb = array(), $sizes = array()/*, $remoto = false, $dir_remoto = ""*/)
{
    ...
    move_uploaded_file($file_tmp_name, $caminho);
    if ($thumb) {
 ...
 * @return string, o nome do arquivo resultante
 */
function uploadBanner($file,$filetpm, $dir_file, $nome, $thumb = false, $dir_thumb = array(), $sizes = array())
{
    $nome_imagem = $file;
    preg_match('/\.(gif|png|jpg|jpeg|bmp){1}$/i', $nome_imagem[name], $ext);
    $ext = isset($ext[1]) ? strtolower($ext[1]) : '';
    ...
    if ($thumb) {
        for ($i = 0; $i < count($dir_thumb); $i++) {
            $caminho_thumb = $dir_file . $dir_thumb[$i] . $nome_imagem_formatado;
            CroppedThumbnail($caminho, $caminho_thumb, $sizes[$i][0], $sizes[$i][1]);
        }
    }
    return $nome_imagem_formatado;
}
