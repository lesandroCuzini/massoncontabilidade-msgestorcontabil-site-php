    ...
    private function prepareFiles() {
        ...
        if ($validate) {
            ...
            for ($i = 0; $i < count($field['name']); $i++) {
                ...
                $metas['name'] = $this->options['title'].'-tmp.'.$metas['extension']; /*$this->generateFileName($this->options['title'], array(
                    'name' => substr($metas['old_name'], 0, (!empty($metas['extension']) ? -(strlen($metas['extension']) + 1) : strlen($metas['old_name']))),
                    'size' => $metas['size'],
                ...
                if (!$is_file_removed && $this->validate(array_merge($metas, array(
                        'index' => $i,
                        'tmp' => $tmp_name
                    ))) && $this->uploadFile($tmp_name, $metas['file'])
                ) {
