...
class TipoLinkUtel extends TipoLinkUtelModel {
    
    private $pagina_retorno = 'tipos_links_uteis-form.html';
    ...
    public function getListagemAdmin() {
        ...
        foreach($lista_resultados as $object) {
            $object->icon_tipo_link_utel = ($object->icon_tipo_link_utel != "") ? "<img src='".UPLOADS_URL."/tipos-links-uteis/".$object->icon_tipo_link_utel."' width='40'>" : ' - ';
    ...
    */
    public function insert() {
        ...
        $this->setNomeTipoLinkUtel(getRequest("nome_tipo_link_utel"));
        if (isset($_FILES['icon_tipo_link_utel']) && $_FILES['icon_tipo_link_utel']['tmp_name'] != "") {
            $imagem_bg = uploadArquivo($_FILES["icon_tipo_link_utel"], 'uploads/tipos-links-uteis/', 'icon_tipo_link_uteis', false, array(''), array());
            $this->setIconTipoLinkUtel($imagem_bg);
        }
        $this->setCorTipoLinkUtel(getRequest("cor_tipo_link_utel"));
