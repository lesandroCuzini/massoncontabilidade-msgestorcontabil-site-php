...
class MinifyFile {
    ...
    function __construct ($debug = false, $screen = false, $base_url = '') {
        $this->_debug = $debug;
        $this->_screen = $screen;
        ...
        if ($base_url == "") {
            ...
            $this->_base_url['dirname'] = $base_url;
        }
    ...
    public function render ( $return = false ) {
        ...
        $header  = '/** '.PHP_EOL;
        $header .= " * Innovare Company ".PHP_EOL;
        $header .= ' * File private - not use'.PHP_EOL;
        $header .= ' * http://www.innovarecompany.com.br'.PHP_EOL;
        $header .= ' * @author Dheyson Wildny (contato@innovarecompany.com.br)'.PHP_EOL;
    ...
    private function run () {
        ...
        if (count($this->_files) === 0) {
            trigger_error('Innovare Company Minify: There\'s no files to be minified!', E_WARNING);
            exit;
        }
        ...
        foreach ($this->_files as $file) {
            ...
            $this->_minified[] = $file;
            $minified_content = '';
            $inf['dirname'] = ($inf['dirname'] == '/' ? '' : $inf['dirname']);

            if (!$is_minified) {
                $minified_content = $this->_debug === false ? $this->minify( $this->_base_url['dirname'].$inf['dirname'].'/'.$inf['basename'] ) : @file_get_contents( $this->_base_url['dirname'].$inf['dirname'].'/'.$inf['basename'] );
                $minified_content = strpos($this->_type, 'css') !== false ? $this->fix_path( $minified_content, $this->_base_url['dirname'].$inf['dirname'].'/'.$inf['basename']) : $minified_content;
            } else {
                $minified_content = @file_get_contents($this->_base_url['dirname'].$inf['dirname'].'/'.$inf['basename']);
            }
    ...
    private function do_uglify ( $raw_code ) {
        if (!function_exists('curl_init')) {
            trigger_error('Innovare Company Minify: Impossible to uglify your code, this PHP instance does not have CURL.');
            return $raw_code;
        }
