...
class Newsletter extends NewsletterModel {
    ...
    private $pagina_retorno = 'newsletter-form.html';
    ...
    public function postProcess($acao) {
            ...
            $id = $this->insert();
                if ($id) {
                    ...
                    if ($acao_posterior == "new") {
                        header("Location: " . getRequest("pagina_retorno") . "?msg=ok");
                    } else {
                        header("Location: " . getRequest("pagina_retorno") . "?msg=ok&id=" . $id);
                    }
                } else {
                    header("Location: " . getRequest("pagina_retorno") . "?msg=erro");
                }
                die;
        }
        ...
        elseif ($acao == "add_news_site") {
            $email = getRequest('email');
            if($this->verificaEmailCadastrado($email) > 0){
                exit('email_existente');
            }
            $id = $this->insert();
            if ($id) {
                exit('ok');
    ...
    public function verificaEmailCadastrado($email){
        return parent::verificaEmailCadastrado($email);
    }
