...
class Servico extends ServicoModel {
    ...
    public function postProcess($acao) {
        ...
        elseif ($acao == "apagar-banner-servico"){
            $id_servico = getRequest('id_servico');
            $campo = getRequest('campo');
            $update = $this->updateCamposImagem($id_servico,$campo,'');
            if($update){
                exit('1');
            }
            exit('0');
    ...
    public function getListagemAdmin() {
        ...
        foreach($lista_resultados as $object) {
            $object->icon_servico = ($object->icon_servico != "") ? "<img src='".UPLOADS_URL."/servicos/".$object->icon_servico."' width='30'>" : ' - ';
    ...
    */
    public function insert() {
        ...
        if (isset($_FILES['icon_servico']) && $_FILES['icon_servico']['tmp_name'] != "") {
            $imagem_bg = uploadArquivo($_FILES["icon_servico"], 'uploads/servicos/', 'icon_servico', false, array(''), array());
            $this->setIconServico($imagem_bg);
        }
        if (isset($_FILES['url_banner']) && $_FILES['url_banner']['tmp_name'] != "") {
            $imagem_bg = uploadArquivo($_FILES["url_banner"], 'uploads/servicos/', 'banner', true, array('banners/'), array(array(1920,500)));
            $this->setUrlBanner($imagem_bg);
        }
        $this->setNomeServico(getRequest("nome_servico"));
        $this->setBreveDescricaoServico(getRequest("breve_descricao_servico"));
        $this->setDescricaoServico(addslashes($_POST['descricao_servico']));
     ...
     */
    public function getServicosHome()
    {
        return parent::getServicosHome();
    }
    /**
     ...
     * @return bool
     */
    public function updateCamposImagem($id_servico, $campo, $valor)
    {
        return parent::updateCamposImagem($id_servico, $campo, $valor);
    }
}
