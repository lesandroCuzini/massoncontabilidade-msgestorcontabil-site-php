...
class Parceiro extends ParceiroModel {
    
    private $pagina_retorno = 'parceiros-form.html';
    ...
    public function getListagemAdmin() {
        ...
        foreach($lista_resultados as $object) {
            $object->imagem_parceiro = ($object->imagem_parceiro != "") ? "<img src='".UPLOADS_URL."/parceiros/".$object->imagem_parceiro."' width='100'>" : ' - ';
    ...
    */
    public function insert() {
        $this->setNomeParceiro(getRequest("nome_parceiro"));

        $linkParceiro = $_POST['link_parceiro'];
        if (substr($linkParceiro, 0, 4) != "http" && $linkParceiro != "#" && $linkParceiro != "") {
            $linkParceiro = "http://" . $linkParceiro;
        }

        $this->setLinkParceiro($linkParceiro);
        $this->setAltImagem(getRequest("alt_imagem"));
        if (isset($_FILES['imagem_parceiro']) && $_FILES['imagem_parceiro']['tmp_name'] != "") {
            $imagem_bg = uploadArquivo($_FILES["imagem_parceiro"], 'uploads/', 'parceiro', true, array('parceiros/'), array(array(250, 120)));
            $this->setImagemParceiro($imagem_bg);
        }
