...
class Parceiro extends ParceiroModel
{
    ...
    public function insert()
    {
        ...
        $linkSite = getRequest('link_site');
        if ($linkSite == "#" || $linkSite == "") {
            $linkSite = "";
        } else {
            if (substr($linkSite, 0, 4) != "http") {
                $linkSite = "http://" . $linkSite;
            }
        }
        $this->setLinkSite($linkSite);

        if (isset($_FILES['url_imagem']) && $_FILES['url_imagem']['tmp_name'] != "") {
            $nome_imagem = 'parceiro_' . date('YmdHis') . str_replace(".", "", substr((string)microtime(), 1, 8));
            $imagem = uploadBanner($_FILES["url_imagem"], $_FILES['url_imagem']['tmp_name'], 'uploads/parceiros/', $nome_imagem, true, array('large/'), array(array(250, 120)));
